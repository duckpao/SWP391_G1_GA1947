    ...
    try (CallableStatement cs = connection.prepareCall(sql)) {
        ...
        if (rs.next()) {
            ...
            if (count > 0) {
                ...
                NotificationWebSocket.broadcastToAll(title, message, notificationType);
                System.out.println("✓ Broadcasted via WebSocket to all users");
            }
...
// Gửi thông báo đến một user cụ thể - with debug
    ...
    try (PreparedStatement ps = connection.prepareStatement(sql)) {
        ...
        if (rows > 0) {
            ...
            NotificationWebSocket.notifyUser(receiverId, title, message, notificationType);
            System.out.println("✓ Notified user " + receiverId + " via WebSocket");
        }
    ...
    try (CallableStatement cs = connection.prepareCall(sql)) {
        ...
        if (rs.next()) {
            boolean success = rs.getInt("updated") > 0;
            
            if (success) {
                // ✅ Update count via WebSocket
                NotificationWebSocket.updateUnreadCount(userId);
                System.out.println("✓ Updated unread count for user " + userId);
            }
...
// 4. markAllAsRead - cập nhật count qua WebSocket
public boolean markAllAsRead(int userId) {
    ...
    try (PreparedStatement ps = connection.prepareStatement(sql)) {
        ...
        if (success) {
            // ✅ Update count via WebSocket
            NotificationWebSocket.updateUnreadCount(userId);
            System.out.println("✓ Updated unread count after delete for user " + userId);
        }
    ...
    public List<Notification> getLatestNotificationsByLimit(int userId, int limit) {
        List<Notification> notifications = new ArrayList<>();

        // SQL với TOP để lấy N bản ghi đầu tiên
        String sql = "SELECT TOP (?) n.*, u.username as sender_username "
                + "FROM Notifications n "
                + "LEFT JOIN Users u ON n.sender_id = u.user_id "
                + "WHERE n.receiver_id = ? "
                + "AND (n.expires_at IS NULL OR n.expires_at > GETDATE()) "
        ...
        System.out.println("Executing getLatestNotificationsByLimit with userId=" + userId + ", limit=" + limit);
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            System.err.println("SQLException in getLatestNotificationsByLimit:");
            e.printStackTrace();
        }
