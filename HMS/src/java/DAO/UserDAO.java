...
public class UserDAO extends DBContext {
    ...
    String sql = "SELECT * FROM Users WHERE (email = ? OR username = ?) AND password_hash = ? AND is_active = 1";
    try {
        PreparedStatement ps = connection.prepareStatement(sql);
        ps.setString(1, emailOrUsername); // Email or Username
        ps.setString(2, emailOrUsername); // Email or Username
        ps.setString(3, password); // Password
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
            User u = mapRow(rs); // Convert row to User object
            return u;
        }
    } catch (Exception e) {
        e.printStackTrace();
