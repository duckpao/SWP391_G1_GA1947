...
<html lang="vi">
    ...
    <body>
        ...
        <script>
            // THAY THẾ TOÀN BỘ PHẦN JAVASCRIPT trong chat.jsp bằng code này:
...
(function() {
        ...
        switch (data.type) {
            case 'message':
                ...
                if (isFromSelectedUser && !isMyOwnMessage) {
                    // Tin nhắn từ user đang chat
                    displayMessage(data);
                    // Đánh dấu là đã đọc ngay lập tức qua WebSocket
                    markMessagesAsReadViaWebSocket(data.senderId);
                } else if (!isMyOwnMessage) {
                    // Tin nhắn từ user khác (không phải đang chat)
                    // Tăng unread count
                    if (!unreadCounts[data.senderId]) {
                        unreadCounts[data.senderId] = 0;
                    }
                    unreadCounts[data.senderId]++;
                    updateUnreadBadges();
                    console.log('Unread count updated for user:', data.senderId, 'count:', unreadCounts[data.senderId]);
                }
                break;
                
            case 'messagesRead':
                // Người khác đã đọc tin nhắn của mình
                console.log('Messages read notification received. ReadBy:', data.readBy, 'MessagesFrom:', data.messagesFrom);
                if (data.messagesFrom == CURRENT_USER_ID && data.readBy == selectedUserId) {
                    // Chỉ update nếu đang chat với người đó
                    updateAllMessagesToRead(data.readBy);
                }
                break;
                
            case 'markReadSuccess':
                console.log('Mark read success for sender:', data.senderId);
    ...
    function displayMessage(data) {
        ...
        let messageClass = 'message ' + (isSentByMe ? 'sent' : 'received');
        
        // Thêm class unread nếu tin nhắn chưa đọc
        if (!isSentByMe && data.isRead === false) {
            messageClass += ' unread';
        }
        ...
        // Thêm data attribute để biết tin nhắn gửi cho ai
        if (isSentByMe) {
            messageDiv.setAttribute('data-receiver-id', data.receiverId || selectedUserId);
        }

        const time = new Date(data.timestamp).toLocaleTimeString('vi-VN');
        
        // Thêm read status cho tin nhắn đã gửi
        let readStatusHtml = '';
        if (isSentByMe) {
            // Kiểm tra isRead: true = đã xem, false/undefined = chưa xem
            const isRead = data.isRead === true;
            const iconClass = isRead ? 'fas fa-check-double read-icon' : 'fas fa-check unread-icon';
            const statusText = isRead ? 'Đã xem' : 'Đã gửi';
            readStatusHtml = '<div class="message-read-status"><i class="' + iconClass + '"></i> <span class="status-text">' + statusText + '</span></div>';
        }
        
        messageDiv.innerHTML = 
            '<div>' + data.content + '</div>' +
            '<div class="message-time">' + time + readStatusHtml + '</div>';
    ...
    function sendMessage() {
        ...
        if (content && selectedUserId && websocket && websocket.readyState === WebSocket.OPEN) {
            ...
            displayMessage({
                ...
                timestamp: timestamp,
                isLocal: true,
                isRead: false // Tin nhắn mới gửi chưa được đọc
            });
        ...
        fetch('${pageContext.request.contextPath}/getUserList')
            ...
            .then(function(users) {
                ...
                users.forEach(function(user, index) {
                    ...
                    // Thêm unread badge
                    const unreadBadgeHtml = '<span class="unread-badge" style="display: none;" data-user-id="' + user.user_id + '">0</span>';
                    
                    userDiv.innerHTML =
                        '<span class="status-indicator status-online"></span>' +
                        '<div style="flex: 1;"><strong>' + user.username + '</strong><br>' +
    ...
    function loadUnreadCounts() {
        fetch('${pageContext.request.contextPath}/chat?action=getUnreadCounts&userId=' + CURRENT_USER_ID)
            .then(function(response) {
                return response.json();
            })
    ...
    function updateUnreadBadges() {
        console.log('Updating badges with counts:', unreadCounts);
        
        // Ẩn tất cả badges trước
        document.querySelectorAll('.unread-badge').forEach(function(badge) {
            badge.style.display = 'none';
        });
        
        // Hiển thị badge cho users có tin nhắn chưa đọc
        Object.keys(unreadCounts).forEach(function(userId) {
            const count = unreadCounts[userId];
            const badge = document.querySelector('.unread-badge[data-user-id="' + userId + '"]');
            console.log('Updating badge for user', userId, 'count:', count, 'badge found:', !!badge);
            
            if (badge && count > 0) {
                badge.textContent = count > 99 ? '99+' : count;
                badge.style.display = 'flex';
                console.log('Badge displayed for user', userId);
            }
        });
    }
    ...
    function selectUser(userId, username) {
        ...
        // Reset unread count ngay lập tức (optimistic update)
        ...
        // Đánh dấu tin nhắn là đã đọc qua WebSocket
        markMessagesAsReadViaWebSocket(userId);
    }

    function markMessagesAsReadViaWebSocket(senderId) {
    if (websocket && websocket.readyState === WebSocket.OPEN) {
        console.log('Marking messages as read via WebSocket: receiver=' + CURRENT_USER_ID + ', sender=' + senderId);
        
        // Lấy số tin nhắn chưa đọc từ user này
        const unreadCountFromThisUser = unreadCounts[senderId] || 0;
        ...
        // Giảm badge count ngay lập tức (optimistic update)
        if (unreadCountFromThisUser > 0 && window.decreaseChatBadge) {
            console.log('Decreasing badge by', unreadCountFromThisUser);
            window.decreaseChatBadge(unreadCountFromThisUser);
        }
    }
}

    function updateAllMessagesToRead(receiverId) {
        console.log('Updating all messages to read for receiver:', receiverId);
        
        // Cập nhật tất cả tin nhắn gửi cho receiverId thành đã đọc
        const messages = document.querySelectorAll('.message.sent[data-receiver-id="' + receiverId + '"]');
        console.log('Found', messages.length, 'messages to update');
        
        messages.forEach(function(messageDiv) {
            const readStatus = messageDiv.querySelector('.message-read-status');
            if (readStatus) {
                const icon = readStatus.querySelector('i');
                const statusText = readStatus.querySelector('.status-text');
                
                if (icon) {
                    icon.className = 'fas fa-check-double read-icon';
                }
                if (statusText) {
                    statusText.textContent = 'Đã xem';
                }
                ...
                if (messages.length === 0) {
                    ...
                    messages.forEach(function(msg) {
                        ...
                        displayMessage({
                            ...
                            isRead: msg.read // Quan trọng: load đúng trạng thái từ DB
                        });
                    });
                }
